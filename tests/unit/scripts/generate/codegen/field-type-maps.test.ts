...
describe("fieldDefsFor", () => {
  it("projects slug/type/required and drops labels", () => {
    expect(
      fieldDefsFor([{ name: "title", label: "Title", type: { type: "text" }, required: true } as never]),
    ).toEqual([{ name: "title", type: { type: "text" }, required: true }]);
  });
});
...
describe("emitters", () => {
  it("emits blockFieldTypes plus the collection-list registry", () => {
    ...
    expect(file).toContain('export const collectionListBlocks: Record<string, string> = {\n  "works-grid": "works"\n};');
  });
